...
cor.sel.otai.pic <- lapply(deta.vs.wi.split.by.codon.for.tgcn,function(x)
    {
       ...
       tmp <- calculateAcrossSpeciesCorrelations(x,tree=tree.to.use,x.var="Relative.tGCN",y.var="Selection",approach="standard",method="spearman")
       return(tmp)
...
trna.phi.deta.target.pic.gc.wide <- trna.phi.deta.target.pic.gc %>%
  dplyr::select(Codon,Gene,Phi.pic) %>%
  pivot_wider(id_cols=Codon,names_from=Gene,values_from=Phi.pic)

mod.vs.mod <- ggplot(trna.phi.deta.target.pic.gc,aes(x=GC.pic,y=Phi.pic)) +
           geom_point() +
           xlab("GC% (PIC)") +
           ylab(expression("Predicted gene expression (PIC)")) +
           theme_cowplot() +
...
trna.phi.deta.tgcn.target <- trna.phi.deta %>% 
  ...
  group_by(Codon) %>%
  mutate(across(c(IKI3,ELP2,ELP3,GC,tGCN,Sphi),~(. - mean(.,na.rm = T))/sd(.,na.rm=T))) %>%
  ungroup()
...
trna.phi.deta.phylolm <- trna.phi.deta.tgcn.target %>%
  ...
  purrr::map(function(x)
    {
      plm.lambda <- phylolm(Selection ~ IKI3 + ELP2 + ELP3 + Sphi +  tGCN + GC,
                            data= x %>% column_to_rownames("Species"), 
                            phy = tree.to.use,
                            model = "lambda")
...
trna.mod.sel
savePlot("../Figures/All_species/sel_vs_trna_mod_expression_pgls.pdf",plot=trna.mod.sel,family = "sans",width=8,height = 4)
```

### Inosine

```{r}
trna.phi.deta.ino.split <- trna.phi.deta.inosine %>%
  mutate(Gene=fct_relevel(Gene,c("TAD2","TAD3"))) %>%
  split(f = ~Gene + Codon)


full.cor <- trna.phi.deta.ino.split %>%
  purrr::map(~cor.test(.x$Phi.pic,.x$Selection.pic,method="spearman") %>% tidy()) %>%
  bind_rows(.id="Gene.Codon") %>%
  separate(col = Gene.Codon,into=c("Gene","Codon"),sep="\\.",remove = F) %>%
  mutate(Gene=fct_relevel(Gene,c("IKI3","ELP2","ELP3")),
...
trna.phi.deta.tgcn.target <- trna.phi.deta %>% 
  ...
  pivot_wider(names_from="Gene",values_from = "Phi") %>%
  group_by(Codon) %>%
  mutate(C.favored = ifelse(Selection < 0, "Yes","No"),
    across(c(TAD2,TAD3,GC,Sphi,tGCN),~(. - mean(.,na.rm = T))/sd(.,na.rm=T))
         ) %>%
  ungroup()
...
trna.phi.deta.phylolm <- trna.phi.deta.tgcn.target %>%
  ...
  purrr::map(function(x)
    {
      plm.lambda. <- phylolm(Selection ~ TAD2 + TAD3 + tGCN + GC + Sphi ,
                            data= x %>% column_to_rownames("Species"), 
                            phy = tree.to.use,
                            model = "lambda")
      print(cor(plm.lambda$fitted.values,plm.lambda$residuals,method="spearman"))
      model.sum <- summary( plm.lambda )
      print(model.sum$adj.r.squared)
      model.coef <- model.sum$coefficients
      df <- model.coef %>% 
        as.data.frame() %>% 
        rownames_to_column("Coefficient") %>% 
...
The results in this section illustrate why we chose to control for $s_\phi$ in our regression analyses. This hyperparameter appears to correlate with 

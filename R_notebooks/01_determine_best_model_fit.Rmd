...
k1.phi <- read_csv("~/Labella2019_scripts/phi_k1_shen_etal_ortholog_matrix_2022_07_08.tsv",col_types = cols())  %>% 
       column_to_rownames("Orthogroup") %>%
       rename_with(~cleanColumnNames(.x,fungi.tree$tip.label)) %>%
       rownames_to_column("Orthogroups")

k2.phi <- read_csv("~/Labella2019_scripts/phi_k2_selectionShared_shen_etal_ortholog_matrix_2022_07_08.tsv",col_types = cols())  %>% 
       column_to_rownames("Orthogroup") %>%
       rename_with(~cleanColumnNames(.x,fungi.tree$tip.label)) %>%
       rownames_to_column("Orthogroups")


orthogroups <- read_tsv("~/Labella2019_scripts/shen_etal_ortholog_matrix_2021_12_17.tsv",col_types = cols())
...
library(boot)
library(parallel)
roc.emp.df.split <- roc.emp.df %>% 
  split(f=roc.emp.df$Species)

boot.diff <- mclapply(roc.emp.df.split,
                    function(gene.expr.df)
                    {
                      result <- boot(data=gene.expr.df,statistic = boot.cor.diff.test,R = 500,method="spearman")
                    },
                    mc.cores = 4
)
boot.ci.species <- lapply(boot.diff,
                         function(boot.result)
                         {
                            ci <- boot.ci(boot.result,type="norm")
                            t.test.res <- t.test(boot.result$t)
                            ci.df <- data.frame(Obs.Diff= boot.result$t0,
                                                Mean.Boot.Diff = mean(boot.result$t),
                                                Lower.95.CI = unname(ci$normal[1,2]),
                                                Upper.95.CI = unname(ci$normal[1,3]),
                                                p.value = t.test.res$p.value)
                         }) %>%
  bind_rows(.id="Species")
improved.fits.species <- boot.ci.species %>% 
  filter(Lower.95.CI > 0) %>%
  dplyr::select(Species) %>%
  deframe()

write(improved.fits.species,file = "improved_species_by_gene_expression_bootstrap_2023_03_23.txt",ncolumns = 1)
...
write(improved.fits.species,"../improved_species_by_gene_expression_25_percent_2023_05_03.txt",ncolumns = 1)
...
compare.emp.non.ref <- compare.emp %>% filter(Corr.Diff != 0)
...
corr.diff.plot <- ggplot(compare.emp,aes(x=Divergence,y=Corr.Diff)) +
...
ggsave2("../Figures/All_species/S01_phi_relative_to_refence_species.pdf",plot=corr.diff.plot)
...
roc.emp.df.long <- roc.emp.df %>%
  ...
  filter(Species %in% "yHMPu5000034632_candida_athensensis_160519.max.cds") %>%
  group_by(Species) %>%
  group_split()
...
deta.k1.files <- file.path("/data2/Labella2019/Final_runs/Results_k_1/",species,"run_2","Parameter_est",paste0(species,"_Selection.csv"))
dm.k1.files <- file.path("/data2/Labella2019/Final_runs/Results_k_1/",species,"run_2","Parameter_est",paste0(species,"_Mutation.csv"))
deta.k2.files <- file.path("/data2/Labella2019/Final_runs/Results_k_2_selectionShared/",species,"run_2","Parameter_est","1_Selection.csv")
dm.k2.1.files <- file.path("/data2/Labella2019/Final_runs/Results_k_2_selectionShared/",species,"run_2","Parameter_est","1_Mutation.csv")
dm.k2.2.files <- file.path("/data2/Labella2019/Final_runs/Results_k_2_selectionShared/",species,"run_2","Parameter_est","2_Mutation.csv")

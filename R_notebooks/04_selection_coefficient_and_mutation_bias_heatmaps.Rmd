...
ag.duet <- createLabels("A/G Duet")
ct.duet <- createLabels("C/T Duet")
quart <- createLabels("Quartet")

labels.p <- ag.duet / ct.duet / quart 


sel.mut.codon <- (labels.p | (codon.sel.compare + theme(legend.position = "none")) | codon.constmut.compare) + plot_layout(widths = c(0.1,1,1))
sel.mut.codon
...
sphi <- lapply(sphi.files,function(x)
  {
   varmut.fit <- str_detect(x,"Results_k_2_selectionShared/")
   data.frame(Sphi=as.numeric(readLines(x)),VarMut.Fit = varmut.fit)
  }) %>%
  bind_rows(.id="Species") %>%
  filter((Species %in% improved.fits.species & VarMut.Fit) | (!Species %in% improved.fits.species & !VarMut.Fit))

...
#trna.count <- read_tsv("../Data/labella_trna.tsv")
trna.count <- trna.count %>%
  #dplyr::select(-c(`Species name`,`Major clade`,`Total_tRNA Counts`,`CAG-Ala`,`CAG-Ser`)) %>%
...
rtai.weights <- read_tsv("../Data/tai_optimization_w_phi_2023_05_03_10_restarts_roc_phi_no_cutoff.tsv") %>%
...
# rtai.long <- trna.count.long.split %>%
#   purrr::map(~gettAI.relative.aa(.x$tGCN,
#                                  codon.order.sense,
#                                  s = c(0,0,0,0,rtai.weights %>% filter(Species %in% .x$Species) %>% dplyr::select(-Species,-Num.Genes) %>% unlist() %>% unname()), 
#                                  sking = 0, 
#                                  exclude.ctg = .x$Exclude.CTG[1])) %>%
#   bind_rows(.id="Species")  %>%
#   dplyr::rename(rtAI = tAI)
# 
...
# rtai.wide <- rtai.long %>% 
#   pivot_wider(id_cols=Species,names_from=Codon,values_from = rtAI)
# 
# write_tsv(rtai.wide,"../Data/cope_wi_rtAI_inverse.tsv")
